  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="font-bold text-white text-lg">Master Stock Catalog</h2>
          <p className="text-xs text-slate-400">SKUs available for affiliates to import onto their storefronts.</p>
        </div>
        <button
          type="button"
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="bg-slate-800/60 border border-slate-700 rounded-xl p-5 mb-6 grid grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs text-slate-400 mb-1">Wholesale price (JMD)</label>
            <input type="number" min="0" step="0.01" value={form.wholesalePriceJmd} onChange={update('wholesalePriceJmd')} required className="w-full bg-slate-900 border border-slate-700 rounded-lg px-3 py-2 text-sm text-white" />
          </div>
          <div>
          ...
          <div className="col-span-2">
            ...
            <textarea
              ...
              rows={3}
              placeholder="What affiliates and customers should know about this product…"
            ...
            <tbody className="divide-y divide-slate-700/50">
              {products.map((p) => (
                <tr key={p.id} className={p.isActive ? '' : 'opacity-50'}>
                  ...
                  <td className="px-5 py-3 text-emerald-400 font-bold">J${Number(p.wholesalePriceJmd).toLocaleString()}</td>
