      ...
      <div className="relative bg-slate-900 border border-slate-700 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col">
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-800">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">What this warehouse sells — apply to request access.</p>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4 space-y-2">
          ...
          {products?.map((product) => (
            <div key={product.id} className="flex items-center gap-3 bg-slate-800/60 border border-slate-700 rounded-lg p-3">
              <div className="h-10 w-10 shrink-0 bg-slate-900 border border-slate-700 rounded-lg flex items-center justify-center overflow-hidden">
                {product.imageUrl ? (
              ...
              <p className="text-xs text-emerald-400 font-bold">J${Number(product.wholesalePriceJmd).toLocaleString()}</p>
            </div>
          ))}
        </div>
